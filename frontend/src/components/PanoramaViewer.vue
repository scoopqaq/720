...
import { authFetch, getImageUrl } from '../utils/api'; // [引入工具]
...
const showSceneBar = ref(false);
const projectInfo = ref(null);
const flatScenes = ref([]);
...
const showTip = (msg) => { tipMessage.value = msg; tipVisible.value = true; setTimeout(() => tipVisible.value = false, 2000); };
const currentRoomName = computed(() => { if (currentRoomId.value && roomsConfig.value[currentRoomId.value]) return roomsConfig.value[currentRoomId.value].name; return ''; });
...
const fetchProjectData = async () => {
  try {
    ...
    if (!data.groups || data.groups.length === 0) { alert("该项目没有任何场景"); return; }
    ...
    data.groups.forEach(group => {
      if (group.scenes) {
        group.scenes.forEach(scene => {
          ...
          config[scene.id] = {
            name: scene.name,
            texture: getImageUrl(`${scene.image_url}?t=${new Date().getTime()}`),
            cover: getImageUrl(scene.cover_url || scene.image_url),
            ...
            hotspots: (scene.hotspots || []).map(h => ({
              ...
              icon_url: h.icon_url,
              scale: h.scale,
              use_fixed_size: h.use_fixed_size
            })),
    ...
    if (sceneList.length > 0) initThree(sceneList[0].id);
  } catch (err) { console.error(err); alert("数据加载失败"); }
};

const getThumb = (scene) => getImageUrl(`${scene.cover_url || scene.image_url}?t=s`);
    ...
    if (animate) {
      ...
      gsap.to(nextSphere.material, {
        ...
        onComplete: () => {
          ...
          createHotspots(roomData.hotspots); // [关键] 动画结束后创建热点
      ...
      currentSphere.visible = false;
      activeSphereIndex = activeSphereIndex === 1 ? 2 : 1;
      createHotspots(roomData.hotspots); // [关键]
      isLoading.value = false;
    }
  });
};
...
const createHotspots = (list) => {
  ...
  list.forEach(hData => {
    // 1. 获取图标纹理 URL
    let url = getImageUrl(hData.icon_url || '/static/icons/system/arrow.png');
    
    // 2. 加载纹理 (带缓存)
    if (!iconTextures[url]) {
      iconTextures[url] = textureLoader.load(url);
    }
    
    const texture = iconTextures[url];
    ...
    const mat = new THREE.SpriteMaterial({ 
      ...
      transparent: true, 
      depthTest: false,
      sizeAttenuation: !hData.use_fixed_size // 同步编辑器设置
    });
    ...
    gsap.to(sprite.scale, { x:targetScale.x, y:targetScale.y, z:1, duration: 0.5, ease: "back.out(1.7)" });
  });
};
...
const onPointerDown = (e) => {
  ...
  if (intersects.length > 0) {
    ...
    // 根据类型执行动作
    if (data.type === 'scene') {
      if (roomsConfig.value[data.targetRoom]) loadRoom(data.targetRoom, true);
      else showTip("目标场景未定义");
    } else if (data.type === 'link') {
      if (data.content) window.open(data.content, '_blank');
    } else if (data.type === 'text') {
      alert(data.content || data.text); // 简单弹窗，以后可以美化
    }
  }
};
...
const applyRoomSettings = (data) => {
  ...
  camera.fov = data.fov_default; camera.updateProjectionMatrix();
  if (data.limit_h_min <= -180 && data.limit_h_max >= 180) { controls.minAzimuthAngle = -Infinity; controls.maxAzimuthAngle = Infinity; } else { controls.minAzimuthAngle = data.limit_h_min * (Math.PI / 180); controls.maxAzimuthAngle = data.limit_h_max * (Math.PI / 180); }
  controls.minPolarAngle = (90 - data.limit_v_max) * (Math.PI / 180); controls.maxPolarAngle = (90 - data.limit_v_min) * (Math.PI / 180);
  controls.reset();
  const az = data.initial_heading * (Math.PI / 180); const pl = (90 - data.initial_pitch) * (Math.PI / 180);
  const r = 0.1; camera.position.x = r * Math.sin(pl) * Math.sin(az); camera.position.y = r * Math.cos(pl); camera.position.z = r * Math.sin(pl) * Math.cos(az);
  controls.update();
};

const onMouseWheel = (e) => { e.preventDefault(); const d = roomsConfig.value[currentRoomId.value]; if(!d)return; let f=camera.fov+e.deltaY*0.05; f=Math.max(d.fov_min, Math.min(d.fov_max, f)); camera.fov=f; camera.updateProjectionMatrix(); };
...
const switchScene = (id) => { if(currentRoomId.value===id)return; loadRoom(id, true); showSceneBar.value = false; };

onMounted(() => fetchProjectData());
onBeforeUnmount(() => { cancelAnimationFrame(animationId); window.removeEventListener('resize', onWindowResize); window.removeEventListener('click', closeMenu); if(renderer) renderer.dispose(); });
</script>
...
.top-bar { pointer-events: auto; background: linear-gradient(to bottom, rgba(0,0,0,0.6), transparent); padding: 15px 20px; display: flex; align-items: center; gap: 15px; }
...
.scene-toggle-btn { background: rgba(0,0,0,0.6); color: white; border: 1px solid rgba(255,255,255,0.3); padding: 8px 20px; border-radius: 30px; cursor: pointer; display: flex; align-items: center; gap: 8px; transition: all 0.3s; backdrop-filter: blur(5px); }
.scene-toggle-btn:hover, .scene-toggle-btn.active { background: rgba(255,255,255,0.9); color: #333; }

.scene-bar-overlay { pointer-events: auto; position: absolute; bottom: 80px; left: 0; right: 0; display: flex; justify-content: center; padding: 0 20px; }
.scene-bar-scroll { background: rgba(0,0,0,0.8); backdrop-filter: blur(10px); padding: 10px; border-radius: 12px; display: flex; gap: 12px; overflow-x: auto; max-width: 100%; box-shadow: 0 5px 20px rgba(0,0,0,0.5); }
.scene-thumb { width: 100px; height: 70px; border-radius: 6px; overflow: hidden; position: relative; cursor: pointer; border: 2px solid transparent; flex-shrink: 0; transition: transform 0.2s; }
...
.scene-name-tag { position: absolute; bottom: 0; left: 0; width: 100%; background: rgba(0,0,0,0.6); color: white; font-size: 10px; text-align: center; padding: 2px 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

.tip-toast { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background: rgba(0,0,0,0.7); color: white; padding: 10px 20px; border-radius: 30px; font-size: 16px; }
